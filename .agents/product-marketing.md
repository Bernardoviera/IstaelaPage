# Product Marketing Context

**Document version:** v8
**Last updated:** 2026-08-06

## Product Overview
**One-liner:** Istaela Santos, sexóloga — educação sexual segura para mulheres e famílias, incluindo apoio para conversar com os filhos sobre sexualidade.
**What it does:** Sessão individual com sexóloga para levar informação sexual segura e sem tabu — tanto para a própria mulher (autoconhecimento, dúvidas, desmistificação) quanto para mães/pais que precisam de orientação sobre como conversar com os filhos sobre sexualidade de forma apropriada para cada idade.
**Product category:** Educação sexual / orientação com sexóloga (autoridade profissional, não terapia genérica).
**Product type:** Serviço — sessão individual com profissional. Modalidade: presencial OU online, cliente escolhe.
**Business model:** Sessão avulsa R$180 (confirmado). Pacotes de sessões são plano futuro — copy/estrutura não deve travar em "sessão única" de um jeito que dificulte introduzir pacotes depois.

**PENDENTE — fatos de negócio ainda não confirmados (não inventar, perguntar à Istaela):**
- Duração da sessão
- Cidade/local do atendimento presencial
- Como funciona na prática o atendimento online (plataforma usada?)
- Formas de pagamento aceitas
- Política de cancelamento e reagendamento (prazo mínimo etc.)

## Target Audience
**Target companies:** N/A (B2C).
**Decision-makers:** A própria pessoa decide (B2C).
**Primary use case:** Duas frentes dentro do mesmo público — (1) mulheres em geral buscando informação sexual segura e sem vergonha para si mesmas; (2) mães/pais que não sabem como conversar sobre sexualidade com os filhos e querem fazer isso de forma correta e segura.
**Jobs to be done:**
- Conseguir informação sexual confiável e sem julgamento, vinda de uma autoridade (sexóloga), não de fontes duvidosas
- Aprender a conversar sobre sexualidade com os filhos sem constrangimento e de forma apropriada para a idade
- Desmistificar tabus e vergonha em torno do próprio corpo e sexualidade
**Use cases:**
- Mãe/pai sem repertório para responder perguntas dos filhos sobre corpo/sexualidade
- Mulher que teve educação sexual repressora ou inexistente e quer se informar de verdade
- Adolescente/jovem indiretamente beneficiado por uma mãe mais preparada para conversar (a cliente é a mãe, não o filho)

## Personas
| Persona | Cares about | Challenge | Value we promise |
|---------|-------------|-----------|-------------------|
| Mãe/pai | Proteger e orientar o filho corretamente | Não sabe como puxar o assunto, medo de falar errado ou cedo/tarde demais | Roteiro seguro e apropriado pra idade, com apoio de sexóloga |
| Mulher (autoconhecimento) | Entender o próprio corpo e sexualidade sem vergonha | Cresceu com informação repressora, tabu, ou nenhuma educação sexual | Informação segura, sem julgamento, com autoridade profissional |

## Problems & Pain Points
**Core problem:** Falta de informação sexual segura e confiável — tanto para a própria mulher quanto para levar aos filhos — por causa de tabu, vergonha, ou simplesmente nunca ter tido acesso a isso.
**Why alternatives fall short:**
- Conteúdo genérico da internet não tem autoridade nem é adaptado ao contexto da família
- Escola nem sempre aborda o tema, ou aborda tarde/superficialmente
- Conversar "no improviso" com os filhos gera respostas erradas ou constrangimento que afasta o diálogo
**What it costs them:** Filhos aprendendo sobre sexualidade com fontes erradas (internet, colegas); mulheres carregando vergonha/desinformação por anos; diálogo familiar quebrado sobre um tema essencial.
**Emotional tension:** Medo de "fazer errado", vergonha de não saber, ansiedade de que o filho descubra sozinho e de forma prejudicial.

## Competitive Landscape
**Direct:** Sexólogos(as) e psicólogos especializados em educação sexual — [PENDENTE: diferencial específico frente a eles]
**Secondary:** Terapia familiar genérica que toca sexualidade tangencialmente.
**Indirect:** Conteúdo de internet/redes sociais sobre educação sexual (não personalizado, sem acompanhamento).
[PENDENTE — nomes de concorrentes reais, se houver]

## Differentiation
**Key differentiators:**
- Autoridade de sexóloga (credencial clara, "Sexóloga" no nome já comunica isso)
- Atende duas necessidades no mesmo serviço: informação pra mulher E preparo pra conversar com os filhos
- Tom seguro, educativo, sem julgamento — nem clínico frio nem constrangedor
**How we do it differently:** Sessão individual, adaptada ao que a pessoa/família precisa — não é conteúdo genérico, é orientação com profissional.
**Why that's better:** Informação confiável de quem entende, não "achismo" de internet.
**Why customers choose us:** Formação sólida (ver Proof Points) + origem prática incomum — começou atendendo dúvidas reais de clientes numa loja de lingerie/produtos sensuais, não só na teoria.

## Objections
| Objection | Response |
|-----------|----------|
| "Vou me sentir envergonhada de perguntar isso" | Ambiente seguro e sem julgamento — sexóloga é treinada exatamente pra isso. |
| "Meu filho é muito novo/velho pra esse assunto" | Orientação é adaptada pra idade certa — a sessão ajuda a calibrar isso. |
| "Consigo aprender isso sozinha na internet" | Internet não substitui orientação confiável e personalizada de uma profissional. |

**Anti-persona:** Quem busca conteúdo adulto/erótico — o site é sobre educação sexual segura, não isso (reforçar clareza pra evitar mal-entendido, dado o disclaimer "+18" mencionado pela profissional — provavelmente herdado do Instagram, onde qualquer menção a sexualidade é marcada assim; validar se esse selo faz sentido no site ou se gera confusão).

## Switching Dynamics
**Push:** Vergonha/medo de "fazer errado" ao educar os filhos; frustração de não ter tido informação sexual segura na própria criação.
**Pull:** Autoridade da sexóloga + promessa de conversa segura, sem julgamento.
**Habit:** Evitar o assunto, empurrar pra escola ou pra "quando ficar mais velho".
**Anxiety:** Medo de expor a própria falta de informação; medo de dizer a coisa errada pro filho.

## Customer Language
**How they describe the problem:**
- "não sei como falar isso com meu filho"
- "tive uma criação sem nenhuma educação sexual"
- [PENDENTE — coletar frases reais de clientes/leads]
**How they describe us:** [PENDENTE]
**Words to use:** informação segura, sem tabu, autoridade, orientação, apropriado pra idade, sem julgamento, conversa aberta.
**Words to avoid:** linguagem clínica fria demais, qualquer insinuação de conteúdo erótico/adulto, jargão técnico sem explicação.
**Glossary:**
| Term | Meaning |
|------|---------|
| Educação sexual | Informação sobre corpo, sexualidade e relações, adaptada à idade e contexto |
| Sexóloga | Profissional especializada em sexualidade humana — fonte de autoridade central da oferta |

## Brand Voice
**Tone:** Seguro, educativo, acolhedor, com autoridade — nem clínico/frio nem constrangedor.
**Style:** Direto, claro, sem jargão; fala com adultos (mulheres e pais), nunca infantiliza.
**Personality:** Segura, confiável, acolhedora, presente, sem julgamento.

## Proof Points
**Metrics:** +800 eventos femininos realizados; +5.000 mulheres impactadas diretamente.
**Customers:** Participante e palestrante no XIX CBSH (Congresso Brasileiro de Sexualidade Humana / Jornada Mineira de Sexologia, 2024, Belo Horizonte) — evento científico de referência na área.
**Testimonials:** 1 depoimento real recebido via WhatsApp (encaminhado pela cliente), usado no site resumido/sem nome real ("Renata, 38 anos — nome alterado a pedido da cliente"). Conteúdo original fala de terapia sexual pessoal (o produto anterior, não o atual de educação familiar) — usada só a parte genérica (autoconhecimento, confiança, guia), evitando o trecho específico sobre vida sexual conjugal pra não confundir o posicionamento atual. Mais depoimentos (idealmente já no foco família/educação) ainda bem-vindos.
**Formação (credenciais):**
- Especialização em Sexologia
- Especialização em Terapia Sexual
- Especialização em Comportamento Humano
- Formação em Recursos Humanos
- Certificação em Programação Neurolinguística (PNL)
- Certificação em Prevenção à Pedofilia e Abuso Infantil — Academia de Polícia Civil
**Origem/história:** Começou dentro de uma loja de lingerie e produtos sensuais — foi ali, ouvindo dúvidas reais de clientes, que percebeu que faltava conhecimento, não produto. É a origem prática (não só acadêmica) da autoridade dela.
**Value themes:**
| Theme | Proof |
|-------|-------|
| Autoridade profissional | 6 formações/certificações + presença em congresso científico nacional (CBSH) |
| Escala de experiência real | +800 eventos, +5.000 mulheres impactadas |
| Segurança específica pra pais | Certificação em Prevenção à Pedofilia e Abuso Infantil (Academia de Polícia Civil) — relevante direto pro público de mães/pais |
| Conversa em família mais segura | [PENDENTE — depoimento] |

## Goals
**Business goal:** Vender sessões avulsas (R$180) agora; evoluir para pacotes futuramente.
**Conversion action:** Clique direto pro WhatsApp (+55 31 9788-0019) em todos os CTAs "Agendar sessão" — confirmado, não é mais placeholder.
**Current metrics:** N/A (novo projeto).

## Changelog
*Newest first. One line per revision: what changed and why.*
- v8 (2026-08-06) — Duas dúvidas de FAQ resolvidas (sessão é com o responsável, não a criança; o que acontece após chamar no WhatsApp). Consolida em um só lugar os 5 fatos de negócio ainda pendentes (duração, cidade, funcionamento online, pagamento, cancelamento) — não inventados, aguardando resposta da Istaela.
- v7 (2026-08-06) — Número de WhatsApp confirmado (+55 31 9788-0019), todos os CTAs "Agendar sessão" agora linkam direto pra lá. Corrigido bug real: páginas legais mostravam "[PENDENTE]" e referência a arquivo interno (.agents/product-marketing.md) publicamente — reescritas com texto real.
- v6 (2026-08-05) — Primeiro depoimento real inserido (resumido, nome alterado). Testimonials deixa de estar pendente em Proof Points.
- v5 (2026-07-31) — Bio real, credenciais (6 formações/certificações) e números de autoridade (+800 eventos, +5.000 mulheres, presença no CBSH 2024) fornecidos pela profissional. Proof Points deixa de estar pendente. Fotos de congresso recebidas para curadoria (algumas aprovadas, uma rejeitada por mostrar produtos sexuais explícitos em quadro — contradiz posicionamento familiar).
- v4 (2026-07-24) — Reposicionamento completo: produto deixa de ser "Reconexão e Sexualidade" (encontro íntimo pessoal) e passa a ser educação sexual com sexóloga para mulheres e famílias (apoio para conversar com filhos). Nome da profissional confirmado: Istaela Santos. Preço confirmado: R$180/sessão. Público, dor, diferenciação e voz todos reescritos para o novo foco.
- v3 (2026-07-24) — Confirmado modelo de venda: sessão avulsa agora, pacotes futuramente. Copy/estrutura do site não deve travar em "sessão única" de forma que dificulte introduzir pacotes depois.
- v2 (2026-07-24) — Confirmado modalidade (presencial + online); preço e CTA final seguem pendentes até usuário confirmar com a profissional.
- v1 (2026-07-24) — Documento inicial, rascunhado a partir do brief de "Reconexão e Sexualidade" fornecido pelo usuário; vários campos de negócio (preço, conversão, provas, concorrência) pendentes de confirmação.
